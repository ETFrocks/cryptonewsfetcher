...
CRYPTO_KEYWORDS=("bitcoin" "ethereum" "uniswap")
STOCKS_KEYWORDS=("coinbase" "paypal")
...
fetch_alphavantage() {
    ...
    echo "https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${keyword}&time_from=$(date -u -d '1 day ago' +%Y%m%dT%H%M)&apikey=$API_KEY_ALPHAVANTAGE"
...
fetch_newsapi() {
    ...
    echo "https://newsapi.org/v2/everything?q=${keyword}&language=en&sortBy=publishedAt&pageSize=3&apiKey=$API_KEY_NEWSAPI_ORG"
...
fetch_gnews() {
    ...
    echo "https://gnews.io/api/v4/search?q=${keyword}&lang=en&max=3&token=$API_KEY_GNEWS"
